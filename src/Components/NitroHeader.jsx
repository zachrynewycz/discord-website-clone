        ...
        <section style={{background: "linear-gradient(90deg, #b473f5, #e292aa)"}} className="min-w-[420px] relative">
            ...
            <div className="max-w-[1200px] mt-20 px-6 relative z-10 md:px-24 lg:grid lg:grid-cols-12 lg:grid-rows-3 lg:mx-auto xl:px-0 pb-36">
                ...
                <div className="flex flex-col md:flex-row mt-10 justify-center md:gap-5 row-start-3 col-span-8 lg:justify-start lg:items-start lg:mt-9 lg:gap-12">
                    <button className="font-whitney_regular text-white text-xl bg-not-quite-black mb-10 rounded-full py-3 px-7 flex items-center justify-center hover:shadow-2xl ease-in duration-100 hover:bg-dark-not-black">$99.99 per year <span className="bg-brand text-xs rounded-full px-2 ml-2 py-1">16% off</span></button>
                    <button className="font-whitney_regular text-white text-xl bg-not-quite-black rounded-full py-3 px-7 mb-10 hover:shadow-2xl ease-in duration-100 hover:bg-dark-not-black">$9.99 per month</button>
                </div>
            </div>

            <div className="absolute top-0 w-full h-full overflow-hidden">
                <video className="hidden lg:block min-w-[140%] min-h-[130%] -left-[20%] absolute top-1/2 -translate-y-[37%]" loop autoPlay={true} src={`${process.env.PUBLIC_URL}/images/nitro-video.mp4`}/>
            </div>
        </section>
    );
}
